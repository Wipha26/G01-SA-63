...
func (uq *UserQuery) QueryDispenses() *DispenseQuery {
	query := &DispenseQuery{config: uq.config}
...
func (uq *UserQuery) WithDispenses(opts ...func(*DispenseQuery)) *UserQuery {
	query := &DispenseQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withDispenses = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withDispenses; query != nil {
		...
		query.Where(predicate.Dispense(func(s *sql.Selector) {
			s.Where(sql.InValues(user.DispensesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_dispenses
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_dispenses" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_dispenses" returned %v for node %v`, *fk, n.ID)
			}
