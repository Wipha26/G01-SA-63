...
func (pq *PatientQuery) QueryDispenses() *DispenseQuery {
	query := &DispenseQuery{config: pq.config}
...
func (pq *PatientQuery) WithDispenses(opts ...func(*DispenseQuery)) *PatientQuery {
	query := &DispenseQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withDispenses = query
	return pq
}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withDispenses; query != nil {
		...
		query.Where(predicate.Dispense(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.DispensesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.patient_dispenses
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "patient_dispenses" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "patient_dispenses" returned %v for node %v`, *fk, n.ID)
			}
